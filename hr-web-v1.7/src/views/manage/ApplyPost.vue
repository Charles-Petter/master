    ...
    <div>
      ...
      <el-form :model="application" :rules="applyRules" ref="application">
        ...
        <el-form-item label="申请原因:" prop="reason_for_application">
          <el-input type="textarea" v-model="application.reason_for_application" style="width: 300px" clearable maxlength="50" show-word-limit></el-input>
        </el-form-item>
        <el-button-group>
...
export default {
  ...
  data() {
    return {
      ...
      application : {
        id : '',
        name : '',
        original_department_name : null,
        new_department_name : null,
        original_post_name : null,
        new_post_name : null,
        is_agree : null,
      ...
      applyRules : {
        new_department_name : [{required : true, message : '请输入新部门名称', trigger : 'blur'}],
        new_post_name : [{required : true, message : '请输入新岗位名称', trigger : 'blur'}],
        reason_for_application : [{required : true, message : '请输入申请原因', trigger : 'blur'}],
      },
  ...
  methods : {
    initEmps(type) {
      ...
      this.$axios.post('/ApplyPostInformation', this.emp).then(resp => {
        this.loading = false;
        if (resp) {
          this.emps = resp.data;
    ...
    changeEditSelect() {
      // 清空部门内容
      this.application.new_post_name = ''

      // 遍历部门的下拉选项数组
      for (const k in this.department_names) {
        // 岗位名称 是否等于 部门名称的下拉选择数组中的某一项
        if (this.application.new_department_name === this.department_names[k]) {
          this.post_type_options = this.post_names[this.application.new_department_name]
        }
      }
    ...
    applyNewPost() {
      ...
      this.$refs['application'].validate(valid => {
        if (valid) {
          this.application.id = this.emp.id;
          this.application.original_department_name = this.emp.department_name;
          this.application.original_post_name = this.emp.post_name;
          // console.log("this.application.new_department_name = ", this.application.new_department_name)
          // console.log("this.application.new_post_name = ", this.application.new_post_name)
          console.log("this.emp.department_name = ", this.emp.department_name)
          console.log("this.emp.post_name = ", this.emp.post_name)
          if (this.application.new_department_name === this.emp.department_name && this.application.new_post_name === this.emp.post_name) {
            Message.error({message : '你已在此部门'});
          } else {
            console.log("申请信息：", this.application);
            this.$axios.post('/ApplyNewPost', this.application).then((resp) => {
              if (resp.data.msg === "申请成功") {
                Message.success({message : '申请成功'});
              } else {
